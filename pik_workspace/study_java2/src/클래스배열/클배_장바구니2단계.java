		...
		public class 클배_장바구니2단계 {
			public static void main(String[] args) {
				...
				while(true) {
					...
					System.out.println("주문한 아이템 갯수가 가장많은 회원은:");
					System.out.println(userList[index].id + " 입니다.");
					...
					for (User user : userList) {
						...
						if (user.id.equals(cart.userId)) {
							for (Item item : itemList) {
								if (cart.itemName.equals(item.name)) {
									total[index] += item.price; 
								}
							}
					...
					System.out.println("주문한 아이템 금액이 가장많은 회원은:");
					System.out.println(userList[index].id + " 입니다.");
