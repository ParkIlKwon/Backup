...
public class 클래스로바꾸기 {
	...
	public static void main(String[] args) {
		...
		// [부서정보]
				...
				depart deArr[] = new depart[deData.length];
				int index = 0;
				for (String[] strings : deData) {
					depart dp = new depart();
					dp.deNo1 = Integer.parseInt(strings[0]);
					dp.deName = strings[1];
					dp.loc = strings[2];
					deArr[index] = dp;
					index ++;
				...
				employee emArr[] = new employee[emData.length];
				index = 0;
				for (String tempe[] : emData) {
					employee emp = new employee();
					emp.no = Integer.parseInt(tempe[0]); 
					emp.name = tempe[1];
					emp.title = tempe[2];
					emp.supNo = tempe[3];
					emp.date = tempe[4];
					emp.sal = Integer.parseInt(tempe[5]);
					emp.com = Integer.parseInt(tempe[6]);
					emp.deNo2 = Integer.parseInt(tempe[7]);
					emArr[index] = emp;
					index++;
				}
				...
				for (depart depart : deArr) {
					if (depart.loc.equals(input)) {
						idata = depart.deNo1 ;
						data2 = depart.deName;
						break;
					}
